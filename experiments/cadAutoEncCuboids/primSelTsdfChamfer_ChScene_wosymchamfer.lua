...
params.visDir = '../cachedir/visualization_chscene_wosymchamfer/' .. params.name
params.visMeshesDir = '../cachedir/visualization_chscene_wosymchamfer/meshes/' .. params.name
params.snapshotDir = '../cachedir/snapshots_chscene_wosymchamfer/' .. params.name
...
if params.usePretrain then
    ...
    local netPretrain = torch.load(paths.concat('../cachedir/snapshots_chscene_wosymchamfer/', params.pretrainNet,'iter' .. tostring(params.pretrainIter) .. '.t7'))
...
local fx = function(x)
    ...
    gradPredTsdfOrig = gradPredTsdf
    gradPredTsdf , flagTsdf =  CheckNaNGrad(gradPredTsdf,params.nParts)
    ...
    --if(params.chamferLossWt > 0) then
      --  errChamfer = chamferCriterion:forward(predParts, dataLoader)
       ...
       -- gradPredChamferOrig = gradPredChamfer
        --gradPredChamfer , flagChamfer = CheckNaNGrad(gradPredChamfer,params.nParts)
    ...
    --if(params.symLossWt > 0) then
      --  errSym = symCriterion:forward(predParts)
        ...
        --gradPredSymOrig = gradPredSym
        --gradPredSym , flagSym = CheckNaNGrad(gradPredSym,params.nParts)
